	...
	return (
		<div className="registBox">
			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						{!thumbFileUrl.length && (
							<td>
								<FilePond filepondEl={thumbFilepondEl} files={thumbFiles} setFiles={setThumbFiles} allowMultiple={false} label="Drag & Drop Image" />
							</td>
						)}
						{thumbFileUrl.length > 0 && (
							<td>
								<div className="previewThumbImgBox">
									<span><img src={thumbFileUrl[0]} alt={thumbFileUrl[0]} /></span>
					...
					<tr>
						<th>Portpolio</th>
						<td>
							<FilePond filepondEl={filepondEl} files={files} setFiles={setFiles} allowMultiple label="Drag & Drop Image" />
			...
			<div className="btnPageBox">
				{!portpolioData ? (
				<button className="btnCustm01" type="button" onClick={() => handleUpload('upload')}>
					<span>Upload Archive</span>
				</button>
				) : (
				<button className="btnCustm01" type="button" onClick={() => handleUpload('edit')}>
					<span>Edit Archive</span>
				</button>
				)}
				{portpolioData && (
				<button className="btnCustm01" type="button" onClick={() => handleDeleteArchive(portpolioData.detailPortpolio._id)}>
